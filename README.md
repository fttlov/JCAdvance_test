...
2. В XboxProfile добавлена возможность эмулировать все кнопки из KMProfile, включая Wheel (об этом ниже). При переключении профилей вызывается нужная функция, хвостов от старого быть недолжно
...
Было замечено, что при, например, SleepTimeOut=8 Wheel для Xbox кнопок часто не срабатывал, аля "пропускаю ход", хотя wheel для KM одновременно с этим работает отлично. Оказалось дело в разнице логики report.wButtons и SendInput, у последнего есть очередь, у первого опрос. У нас иттерация цикла WheelCounter == 1, в моём случае игра проверяет эту структуру с частотой  ~16.6 мс(60fps), при SleepTimeOut=15мс врое как ок, при 8мс ни разу не Ок. С помощью таймера можно искуственно растянуть нажатие эмулируемой кнопки на определённое время в мс, чтобы событие полюбому было зарегестрированно игрой, движом и т.д.

Короче говоря, Wheel для Xbox теперь не зависит от SleepTimeOut (при ввменяемых значениях). Я тестировал таймер 17мс кнопки  всегда срабатывает чётко, хоть при 8, хоть при 4 SleepTimeOut.

Зачем вообще париться? Я ещё давно приметил рваное движение курсора Gyro мышки, сразу понял дело в частоте опроса - 66,67 явно недостаточно. 125hz - самое то.
В коде есть пометки, если что. Вот.
