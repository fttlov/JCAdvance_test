...
Короче говоря, Wheel для Xbox теперь не зависит от SleepTimeOut (при вменяемых значениях). Я тестировал таймер 17мс (1 кадр) - кнопки всегда срабатывают чётко при 8 SleepTimeOut. Можно и подстраховаться, Wheel кнпоки темая явно не про жёсткие тайминги.
