...
6. И самая большая правка - Универсальный блок обработки WHEEL для Xbox и KM + мегатаймер (ууу). Тут сразу две решаются задачи.
а) Чтобы Wheel в профиле XboxProfiles мог эмулировать KM кнопки б) сначала как побочный эффект, появился таймер обработки Xbox кнопок при вызове Wheel.
Что это? Если очень коротко и грубо - отвязка Wheel Xbox от SleepTimeOut

Было замечено, что при, например, SleepTimeOut=8 Wheel для Xbox кнопок часто не срабатывал, аля "пропускаю ход", хотя wheel для KM одновременно с этим работает отлично. Оказалось дело в разнице логики report.wButtons и SendInput, у последнего есть очередь, у первого опрос. У нас иттерация цикла WheelCounter == 1, в моём случае игра проверяет эту структуру с частотой  ~16.6 мс(60fps), при SleepTimeOut=15мс вроде как ок, при 8мс ни разу не Ок. С помощью таймера можно искуственно растянуть нажатие эмулируемой кнопки на определённое время, чтобы событие полюбому было зарегестрированно игрой, движом и т.д. Например, ксли выставить таймер 50мс, он динамически высчитывает, сколько итераций потребуется, чтобы продержать кнопку ровно 50 миллисекунд (например, при Sleep=8 это будет ~6 итераций) и "держит" её в report.wButtons всё это время.

Короче говоря, Wheel для Xbox теперь не зависит от SleepTimeOut (при ввменяемых значениях). Я тестировал таймер 17мс (1 кадр) - кнопки всегда срабатывает чётко при 8 SleepTimeOut. Можно и подстраховаться, Wheel кнпоки темая явно не про жёсткие тайминги.
