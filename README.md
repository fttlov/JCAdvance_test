...
Короче говоря, Wheel для Xbox теперь не зависит от SleepTimeOut (при вменяемых значениях). Я тестировал таймер 17мс (1 кадр) - кнопки всегда срабатывают чётко при  SleepTimeOut 8. Можно и подстраховаться, Wheel кнпоки тема явно не про жёсткие тайминги минимальные задержки, как и игра на геймпаде в целом.
