...
void LoadXboxProfile(std::string ProfileFile) {
	...
	// Keyboard-Mouse emulation for XboxProfiles																			//@003 Эмулируем Клава мышь в XboxProfiles без геммора с профилями
...
int main(int argc, char **argv)
{
	...
	while (!(GetAsyncKeyState(VK_LMENU) & 0x8000 && GetAsyncKeyState(VK_ESCAPE) & 0x8000))
	{
		...
		// AimingMode (мышь / стик)  двухкнопочный bind новый парсинг		//@007
		...
		if (PrimaryGamepad.ControllerType == NINTENDO_JOYCONS) {
			...
			if ((PrimaryGamepad.InputState.buttons&JSMASK_ZR) && CurrentXboxProfile.ZR != XINPUT_GAMEPAD_RIGHT_TRIGGER) {
		...
		// Motion wheel					//@010 Универсальный блок Wheel для XBOX и KM + патч SleepTimeOut для Xbox Wheel теперь после старого Wheel в KM
		/*if (AppStatus.GamepadEmulationMode != EmuKeyboardAndMouse) {
		...
		if ((IsKeyPressed(VK_MENU) && IsKeyPressed('1')) || (IsKeyPressed(VK_MENU) && IsKeyPressed('2')) ||
			...
			// AimingToggleButton (Gyro on\off ) двухкнопочный bind новый парсинг и AimingByPressingMode (больше не жмем Cature 2 раза) в Config	//@012
		...
		bool DontResetInputState = !( // Reset clicks when activating some actions / Сброс нажатий при активации некоторых действий
			...
			!(PrimaryGamepad.InputState.buttons & JSMASK_HOME) || //@013 иначе на HOME не эмулируются кнопки KM в XboxProfile.	
			IsSharePressed ||
			IsRecordPressed);
		...
		if (AppStatus.GamepadEmulationMode == EmuKeyboardAndMouse || AppStatus.GamepadEmulationMode == EmuGamepadEnabled) { //@014  add EmuGamepadEnabled для эмуляции KM в XboxProfiles
			...
			//  Разделене BACK START для SONY и N как у Xbox																//@015
			if (PrimaryGamepad.ControllerType == SONY_DUALSENSE || PrimaryGamepad.ControllerType == SONY_DUALSHOCK4) {
				KeyPress(PrimaryGamepad.ButtonsStates.Back.KeyCode, DontResetInputState && (PrimaryGamepad.InputState.buttons & JSMASK_SHARE), &PrimaryGamepad.ButtonsStates.Back, true);
			...
			// Motion wheel
			/*PrimaryGamepad.ButtonsStates.WheelDefault.IsPressed = false;			//	 Ниже унимверсальный WHEEL
